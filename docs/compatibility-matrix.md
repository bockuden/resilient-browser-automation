...
| --- | --- | --- | --- | --- |
| `main` (unreleased .NET 10 worker) | `1.1.3` / `ghcr.io/bockuden/resilient-automation-test-stand:1.1.3` | 2026-07-28 | Full local Compose Chromium E2E, 172 s; GitHub Actions run is required after this consumer change is published | L1 happy-path pagination; L2 transient `503` + `Retry-After`; L3 protected login, DOM change and duplicates; bonus checkpoint resume, cancellation and terminal evidence |
